function setPixel(x,y) {
  ...
  // start by intersecting with plane representing
  // highest point on scenery
  if (rayDir[1] >= 0.0) { d = MAX_DIST; }
  else { d = -(rayPos[1]-0.5)/rayDir[1]; }
  
  for (; d<=MAX_DIST; d+=incr)
  {
    rayPos[0] += rayDir[0]*incr;
    rayPos[1] += rayDir[1]*incr;
    rayPos[2] += rayDir[2]*incr;
    
    height = rayPos[1] - h( rayPos[0], rayPos[2] );
    
    if (height > 0) {
      // Steepest slope is <pi/8 so we can:
      incr = 0.01 + 2*height; 
    } else break;
